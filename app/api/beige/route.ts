...
import { findBeigeTargets } from '@/lib/beige-finder'
import { rateLimiter } from '@/lib/rate-limiter'

export async function POST(request: NextRequest) {
  console.log("Beige API endpoint called")
  ...
  try {
    ...
    console.log("Starting beige target search")
    
    try {
      const { myNation, targets } = await findBeigeTargets(apiKey, nationId, 10, 10)
      
      // Record the request after successful processing
      rateLimiter.recordRequest(nationId)
      
      console.log("Beige search completed", { 
      ...
      return NextResponse.json({
        targets,
        searchTitle: `Beige Targets for Nation ID ${nationId}`,
        myNation
      })
      
    } catch (error) {
      console.error("Error in beige target search:", error)
    ...
    console.error("Unexpected error in beige API:", error)
